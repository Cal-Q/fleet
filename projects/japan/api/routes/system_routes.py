...
def download_document(category: str, filename: str):
    ...
    drive_path = os.path.abspath(os.path.join(GDRIVE_ASSETS_DIR, category, clean_name))
    if drive_path.startswith(os.path.abspath(GDRIVE_ASSETS_DIR)) and os.path.exists(drive_path):
        return FileResponse(drive_path, filename=clean_name, media_type="application/pdf")
    target_dir = allowed_categories[category]
    safe_path = os.path.abspath(os.path.join(target_dir, clean_name))
    if not safe_path.startswith(os.path.abspath(target_dir)) or not os.path.exists(safe_path):
        raise HTTPException(status_code=404, detail="File non trovato.")
    return FileResponse(safe_path, filename=clean_name, media_type="application/pdf")
