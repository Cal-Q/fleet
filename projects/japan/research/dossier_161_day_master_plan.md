# Dossier di Ricerca Strategica: Piano Deterministico a 161 Giorni per la Borsa MEXT FY2027

**Data di Calibrazione**: 13 Settembre 2026 – 20 Febbraio 2027  
**Candidato**: Patrik Potenza (UniTO L-11, 2° anno triennale, Matricola 993141)  
**Obiettivo Strategico**: Borsa di Studio MEXT Japanese Studies (*Nikken-sei*) — Ambasciata del Giappone a Roma  
**Tempo Totale**: 161 giorni consecutivi | ~23 settimane | ~380 ore nette totali  
**Database di Riferimento**: `/opt/japan/curriculum/master_161_day_curriculum.json`

---

## 1. Quadro Diagnostico di Partenza & Modello Matematico

### 1.1 Ground Truth Iniziale (Giorno 1 • 13 Settembre 2026)
* **Vocabolario & Kanji Maturo (Anki)**: 5.052 carte mature attive (Kurogane deck + Kodansha Kanji 1.099 kanji).
* **Progresso Grammaticale Bunpro**:
  * **N5**: 132 / 132 (100.0% completato)
  * **N4**: 138 / 185 (74.6% completato • **47 punti rimanenti**)
  * **N3**: 7 / 220 (3.2% completato • **213 punti rimanenti**)
  * **N2**: 0 / 221 (0.0% • 40-50 strutture selezionate per Parte B)
* **Performance MEXT Attuale**: Parte A 90% (potenziale), Parte B <40% (mancanza costrutti N3/N2), Parte C <25%.

---

## 2. Architettura a 3 Macro-Cicli (Periodizzazione Scientifica)

```
[13 Set 2026] ─────── FASE 1: Chiusura N4 & Part A (14 gg) ───────> [26 Set 2026]
[27 Set 2026] ─────── FASE 2: Padronanza N3 & Drill Part B (65 gg) ─> [30 Nov 2026]
[01 Dic 2026] ─────── FASE 3: Full Papers & Simulazione Orale (82 gg) > [20 Feb 2027]
```

### FASE 1: Fondamenta & Chiusura N4 (Giorni 1 – 14 • 13 Set – 26 Set 2026)
* **Obiettivo Primario**: Portare Bunpro N4 al 100% (185/185) studiando i 47 punti rimanenti a ritmo di **4 punti/giorno**.
* **Slot 1 (40 min)**: Mantenimento Anki SRS mature a zero arretrati (~35-45 card/die).
* **Slot 2 (45 min)**: Studio dei 4 punti Bunpro odierni (es. `たところだ`, `ながら`, `ているところだ`, `たばかり`).
* **Slot 3 (25 min)**: Frasi d'esempio Bunpro + consolidamento delle 12 coppie Keigo irregolari (come carte lessicali, NO simulazioni orali complesse).
* **Slot 4 (15 min)**: Drill mirato Sezione A (10-12 quesiti di particelle e kanji elementari).
* **Milestone Giorno 14 (26 Set)**: **N4 100% Completato**. Giorno cuscinetto per review generale.

### FASE 2: Padronanza N3 & Drill Selettivo Parte B (Giorni 15 – 79 • 27 Set – 30 Nov 2026)
* **Obiettivo Primario**: Attraversamento completo di 220 punti Bunpro N3 a ritmo di **4 punti/giorno di studio** (con 1 giorno cuscinetto ogni 7).
* **Slot 1 (40 min)**: SRS Anki con innesto vocaboli ed espressioni N3.
* **Slot 2 (45 min)**: Studio intensivo grammatica N3 (connettori logici `わけ`, `にしては`, `反面`, `にすぎない`).
* **Slot 3 (25 min)**: Verbi composti N2 (`～切る`, `～出す`, `～直す`, `～込む`) e causativo-passivo.
* **Slot 4 (25 min)**: Drill Parte B (12-15 quesiti a crocette su grammatica intermedia).
* **Milestone Giorno 79 (30 Nov)**: **N3 100% Completato**. Punteggio target su Parte B: **>55%**.

### FASE 3: Full Past Papers, Skimming & Difesa Orale (Giorni 80 – 161 • 01 Dic 2026 – 20 Feb 2027)
* **Obiettivo Primario**: Padroneggiare la gestione del tempo d'esame (60 minuti integrali A+B+C) e la fluidità orale in Keigo per il colloquio d'ambasciata.
* **Slot 1 (35 min)**: SRS Anki di mantenimento rapido.
* **Slot 2 (40 min)**: Skimming accademico e lettura veloce testi complessi (sociologia urbana, saggi culturali).
* **Slot 3 (35 min)**: Simulazione orale del colloquio d'ambasciata (shadowing audio, motivazione MEXT, progetto Terzi Luoghi e UniTO).
* **Slot 4 (55 min)**: Simulazione d'esame integrale cronometrata (Parti A+B+C con revisione distrattori).
* **Fase Tapering (Giorni 152 – 161 • 11 Feb – 20 Feb 2027)**: Riduzione del carico orario, ripasso leggero delle schede riassuntive, sonno e tenuta psicofisica.
* **Giorno 161 (20 Febbraio 2027)**: **ESAME UFFICIALE MEXT A ROMA**.

---

## 3. Matrice del Punteggio & ROI Orario

| Sezione | Livello JLPT | Punti Max | Target Minimo | Punteggio Atteso | Strategia |
| :--- | :---: | :---: | :---: | :---: | :--- |
| **Parte A** | N5 / N4 | 35 | 32 (91%) | **33–34 (94–97%)** | Zero errori ammessi su particelle e letture. |
| **Parte B** | N3 / N2 | 35 | 18 (51%) | **20–22 (57–63%)** | Chirurgico su Keigo, connettori e composti. |
| **Parte C** | N1 | 30 | 9 (30%) | **10–12 (33–40%)** | Riconoscimento semantico kanji + eliminazione logica. |
| **TOTALE** | — | **100** | **59%** | **63–68%** | **Soglia matematica per l'accesso al colloquio orale.** |

---

## 4. Invarianti e Protocolli di Continuità

1. **Nessun Blocco Superiore a 170 Minuti**: La media giornaliera è di **141.2 minuti (2.35h)**.
2. **23 Giorni Cuscinetto Distribuiti**: Ogni 7 giorni è programmato un buffer per assorbire imprevisti o esami universitari senza accumulare debito di studio.
3. **Audit Automatico Costante**: Verificabile in qualsiasi momento via CLI con `python3 academic/curriculum_inspector_cli.py --audit`.
